...
trait Model
{
   ...
   * @return void
   */
  public static function lazy(int $count, ?Closure $closure = null)
  {
    $page = 1;

    if (Get::has('page')) $page = Get::key('page');
    ...
    if ($closure !== null) {
      ...
      $get = $query->skip($skip)->take($count)->get();
    } else {
      $get  = self::skip($skip)->take($count)->get();
    }
    ...
    $last  = '/?page=' . (strpos(($total / $count),'.') !== false ? explode('.', $total / $count)[0] + 1 : ($total / $count));
